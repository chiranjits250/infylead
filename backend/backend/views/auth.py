...
from rest_framework.response import Response
from backend.custom_exception_handler import CustomException, VagueException
...
from backend.validators import get_validation_errors, is_company_email, normalize_django_errors, validate_password_strength
...
def sign_up(request):
    ...
    elif not is_company_email(data['email']):
        raise CustomException('Please use a company email address.')
...
def me(request):
    if not IsAuthenticated().has_permission(request, None):
        return Response({"is_authenticated": False, "is_banned": False, "is_admin": False, "has_on_boarded": False})
    result = UserSerializer(User.objects.get(id=request.user_id)).data
...
def google_auth(request):
    ...
    name = data['name']
    email = data['email']
    id = get_or_create_google_user(name, email)
