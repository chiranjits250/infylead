...
const Kubernetes = require("@omkar111111/utils/kubernetes")

if (Kubernetes.in_local) {
  const dotenv = require("dotenv")
  dotenv.config({ path: "../dev.env" })
}
function getVariable(key) {
  if (!process.env[key]) {
    throw new Error(`${key} ENVIROMENT VARIABLE not set.`)
  } else {
...
const nextConfig = {
  env: {
    OAUTH_CLIENT_ID: getVariable("OAUTH_CLIENT_ID"),
